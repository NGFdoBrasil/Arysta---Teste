	...
	if(selectedItem.inputName == 'codGrupo'){  
		...
		$("#content").scroll(function(){
			...
			for(var e=0;e<dsGrupo.values.length;e++){            
				...
				var dsGrupoProg = DatasetFactory.getDataset('ARY-sql2dataset-programas', null, [c1,c2],null);
		...
		console.log(dsGrupo.values.length);
		...
		for(var h = 0; h < dsUsuarios.values.length; h++){
			...
			wdkAddChild('tabela_usuarios');

			ultimaLinhaConf = ultimaLinhaConf + 1 ;

			$("#contaUser").html($("#tabela_usuarios tr").length -2);
		...
		for(var m=0; m < dsConflitos.values.length; m++){
			...
			$("#grupoConfito___"+linha).val(dsConflitos.values[m]['GRUPO_CONFLITOS']);
